...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '5118591367').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '1002026968919').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '1002026968919').split()]
...
# MongoDB information
DATABASE_URL = environ.get('DATABASE_URL', "mongodb+srv://telegram:telegrambots@cluster0.wyl1srz.mongodb.net")
...
# Others
IMDB_DELET_TIME = int(environ.get('IMDB_DELET_TIME', "10"))
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '1002026968919')).split()]
